...
We know that the state at the end of the \enquote{then} branch is \((\Glo_1, \Hea_1)\), and the
...
The result of joining the two states is \((\Glo_{new}, \Hea_{new})\), where:
\\[.4em]
...
\item And finally, \pycode|S[Import(name)]| is extended (now it handles a single library):
...
%      hintval := if is_value(evaledhint) then evaledhint else <Execution Halt>
%
